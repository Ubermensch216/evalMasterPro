...
export default function ScoringDashboard({ evaluator, onLogout }: ScoringDashboardProps) {
  const { candidates, items, scores, comments, addScore, addComment } = useStore();
  ...
  const handleSubmit = (candidateId: string) => {
    ...
    // Save scores
    items.forEach(item => {
      addScore(candidateId, evaluator.id, item.id, candidateScores[item.id]);
    });

    // Save comment
    const comment = localComments[candidateId];
    if (comment) {
      addComment(candidateId, evaluator.id, comment);
    }
        ...
        <CardContent>
            <Accordion type="single" collapsible className="w-full">
                {candidates.map(candidate => {
                    ...
                    return (
                        <AccordionItem value={candidate.id} key={candidate.id} disabled={isScored}>
